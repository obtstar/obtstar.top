...
<p>输出长度 8</p>
<p>采样控制 9</p>
<p>和 Top‑P 10 综合应用 1</p>
<p>Putting it all together 11 1</p>
<p>通用提示 / 零样本 13</p>
<p>一次性示例与少量示例 15</p>
<p>系统提示、上下文提示与角色提示 18</p>
<p>角色提示 21</p>
<p>上下文提示 23</p>`,
        'en': `<p>Prompt</p>
<p>Engineering</p>
...
<p>Haymaker</p>
<p>Joey Haymaker</p>
<p>February 2025 2 2025</p>
<p>Prompt engineering 7</p>
<p>LLM output configuration 8</p>
<p>8</p>
<p>Output length 8</p>
<p>Sampling controls 9</p>
<p>Top-K and top-P 10 Top‑K</p>
<p>Putting it all together 11 1</p>
<p>General prompting / zero shot 13</p>
<p>One-shot &amp; few-shot 15</p>
<p>System, contextual and role prompting 18</p>
<p>Role prompting 21</p>
<p>Contextual prompting 23</p>`,
      }
    },
    {
...
<p>使用指令而非限制 56</p>
<p>在提示中使用变量 58</p>
<p>尝试不同的输入格式和写作风格 59</p>
<p>对于带分类任务的少样本提示，混合类别 59</p>
<p>适应模型更新 60</p>
<p>修复 61</p>
<p>使用模式 62</p>
<p>与其他提示工程师一起实验 63</p>
...
<p>Introduction</p>
<p>介绍</p>
<p>在考虑大型语言模型的输入和输出时，文本提示（有时伴随其他模态如图像提示）是模型用来预</p>
<p>测特定输出的输入。你不需要成为数据科学家或机器学习工程师——每个人都可以编写提示。然</p>
<p>而，设计最有效的提示可能很复杂。提示的许多方面都会影响其效果：你使用的模型、模型的训</p>
<p>练数据、模型配置、你的用词选择、风格和语气、结构以及上下文都很重要。因此，提示工程是</p>
<p>一个迭代过程。不充分的提示可能导致模糊、不准确的回答，并且会阻碍模型提供有意义输出的</p>
<p>能力。</p>
<p>年2月 6</p>`,
        'en': `<p>Step-back prompting 25</p>
<p>Chain of Thought (CoT) 29</p>
<p>Self-consistency 32</p>
<p>ReAct (reason &amp; act) 37 ReAct</p>
<p>Automatic Prompt Engineering 40</p>
<p>Code prompting 42</p>
<p>Prompts for writing code 42</p>
<p>Prompts for explaining code 44</p>
<p>Prompts for translating code 46</p>
<p>Prompts for debugging and reviewing code 48</p>
<p>What about multimodal prompting? 54</p>
<p>Best Practices 54</p>
<p>Provide examples 54</p>
<p>Design with simplicity 55</p>
<p>Be specific about the output 56</p>
<p>Use Instructions over Constraints 56</p>
<p>Use variables in prompts 58</p>
<p>Experiment with input formats and writing styles 59</p>
<p>For few-shot prompting with classification tasks, mix up the classes 59</p>
<p>Adapt to model updates 60</p>
<p>JSON Repair 61 JSON</p>
<p>Working with Schemas 62</p>
<p>Experiment together with other prompt engineers 63</p>
<p>Document the various prompt attempts 64</p>
<p>Endnotes 68</p>
<p>Introduction</p>
...
<p>can lead to ambiguous, inaccurate responses, and can hinder the model’s ability to provide</p>
<p>meaningful output.</p>
<p>February 2025 6 2025</p>`,
      }
    },
    {
      title: 'such as temperature etc.',
      title_i18n: {
        'zh-CN': 'such as temperature etc.',
        'en': 'such as temperature etc.',
      },
      level: 1,
...
,
      content_i18n: {
        'zh-CN': `<p>当你与双子座聊天机器人交流时，基本上是在编写提示，但本白皮书重点介绍如何在 Vertex</p>
<p>中或通过 API 为双子座模型编写提示，因为直接提示模型可以让你访问诸如温度等配置。</p>
<p>本白皮书详细讨论了提示工程。我们将探讨各种提示技术，帮助你入门，并分享成为提示</p>
<p>专家的技巧和最佳实践。我们还将讨论在编写提示时可能遇到的一些挑战。</p>
<p>提示工程</p>
<p>记住大型语言模型（LLM）的工作原理；它是一个预测引擎。模型以顺序文本作为输入，然后</p>
<p>根据其训练数据预测下一个应该出现的标记。LLM通过不断重复这一过程来运行，将之前预测</p>
<p>的标记添加到顺序文本的末尾，以预测下一个标记。下一个标记的预测基于之前标记之间的关系</p>
<p>以及LLM在训练中见过的内容。</p>
<p>当你编写提示时，你是在尝试设置LLM以预测正确的标记序列。提示工程是设计高质量提示以</p>
<p>引导LLM生成准确输出的过程。这个过程包括调整以找到最佳提示，优化提示长度，以及评估</p>
<p>提示的写作风格和结构与任务的相关性。在自然语言处理和LLM的背景下，提示是提供给模型</p>
<p>以生成响应或预测的输入。</p>
<p>年2月 7</p>
<p>这些提示可用于实现各种理解和生成任务，例如文本摘要、信息提取、问答、文本分类、</p>
<p>语言或代码翻译、代码生成以及代码文档编写或推理。</p>
<p>欢迎参考谷歌的提示指南2,3，其中包含简单且有效的提示示例。</p>
<p>在进行提示工程时，您将从选择模型开始。无论您使用的是 Vertex AI 中的 Gemini 语言模型、</p>
<p>、Claude，还是开源模型如 Gemma 或 LLaMA，提示都可能需要针对您的特定模型进行</p>
<p>优化。</p>
<p>除了提示词之外，您还需要调整大型语言模型（LLM）的各种配置。</p>
<p>输出配置</p>
<p>一旦选择了模型，您需要确定模型的配置。大多数LLM都带有各种配置选项，用于控制LL</p>
<p>的输出。有效的提示词工程需要为您的任务优化设置这些配置。</p>
<p>输出长度</p>
<p>一个重要的配置设置是响应中生成的标记数量。生成更多标记需要LLM进行更多计算，导致</p>
<p>更高的能耗，可能响应时间更长，且成本更高。</p>
<p>年2月 8</p>
<p>减少大型语言模型（LLM）的输出长度并不会使其生成的内容在风格或文本上更加简洁，它</p>
<p>只是导致模型在达到限制后停止预测更多的标记。如果你的需求需要较短的输出长度，你可能</p>
<p>还需要设计你的提示以适应这一点。</p>
<p>输出长度限制对于某些LLM提示技术尤为重要，比如ReAct，在这些技术中，LLM会在你</p>
<p>想要的响应之后继续生成无用的标记。</p>
<p>请注意，生成更多的标记需要LLM进行更多计算，导致更高的能耗和可能更慢的响应</p>
<p>时间，从而带来更高的成本。</p>
<p>采样控制</p>
<p>大型语言模型（LLM）并不正式预测单个标记。相反，LLM 会预测下一个标记可能是什么的</p>
<p>概率，LLM 词汇表中的每个标记都会获得一个概率。然后根据这些标记概率进行采样，以确</p>
<p>定下一个生成的标记。温度（Temperature）、top‑K 和 top‑P 是最常见的配置设置，用于</p>
<p>决定如何处理预测的标记概率以选择单个输出标记。</p>
<p>Temperature</p>
<p>温度</p>
<p>温度控制标记选择的随机程度。较低的温度适用于期望更确定性响应的提示，而较高的温度则</p>
<p>可能导致更丰富或意想不到的结果。温度为 0（贪婪解码）时是</p>
<p>年2月 9</p>`,
        'en': `<p>When you chat with the Gemini chatbot,1 you basically write prompts, however this</p>
<p>whitepaper focuses on writing prompts for the Gemini model within Vertex AI or by using AI</p>
<p>the API, because by prompting the model directly you will have access to the configuration</p>
<p>such as temperature etc.</p>
...
<p>to the task. In the context of natural language processing and LLMs, a prompt is an input</p>
<p>provided to the model to generate a response or prediction.</p>
<p>February 2025 7 2025</p>
<p>These prompts can be used to achieve various kinds of understanding and generation</p>
...
<p>optimized for your specific model, regardless of whether you use Gemini language models in GPT</p>
...
<p>come with various configuration options that control the LLM’s output. Effective prompt M</p>
<p>engineering requires setting these configurations optimally for your task.</p>
<p>Output length</p>
...
<p>consumption, potentially slower response times, and higher costs.</p>
<p>February 2025 8 2025</p>
<p>Reducing the output length of the LLM doesn’t cause the LLM to become more stylistically</p>
...
<p>can lead to more diverse or unexpected results. A temperature of 0 (greedy decoding) is</p>
<p>February 2025 9 2025</p>`,
      }
    },
    {
      title: 'you may not always get the same output with temper',
      title_i18n: {
        'zh-CN': 'you may not always get the same output with temper',
        'en': 'you may not always get the same output with temper',
      },
      level: 1,
      content: `<p>Prompt Engineering Prompt Engineering</p>
...
,
      content_i18n: {
        'zh-CN': `<p>确定性：始终选择最高概率的标记（但请注意，如果两个标记具有相同的最高预测概率，取决于</p>
<p>如何实现平局决胜，温度为0时您可能不会总是得到相同的输出）。</p>
<p>接近最大值的温度往往会产生更随机的输出。随着温度越来越高，所有标记成为下一个预测标记</p>
<p>的概率变得相等。</p>
<p>的温度控制可以类似于机器学习中使用的softmax函数来理解。较低的温度设置类似于</p>
<p>低softmax温度（T），强调单一的、确定性高的首选温度。较高的Gemini温度设置则像高</p>
<p>温度，使得围绕所选设置的更广泛温度范围变得更可接受。这种增加的不确定性适用于</p>
<p>那些不需要严格、精确温度的场景，例如在尝试创造性输出时。</p>
<p>和 top‑P</p>
<p>和 top‑P（也称为核采样）是大型语言模型中用于限制预测下一个词元仅从预测概率最</p>
<p>高的词元中选择的两种采样设置。与温度参数类似，这些采样设置控制生成文本的随机性和多样</p>
<p>性。</p>
<ul>
<li>采样从模型预测的分布中选择概率最高的前 K 个词元。Top‑K 越高，模型输出越有</li>
</ul>
<p>创造性和多样性；Top‑K 越低，模型输出越保守和事实性。Top‑K 为 1 相当于贪婪解码。</p>
<p>年2月 10</p>
<ul>
<li>采样选择累计概率不超过某个值（P）的最高概率词元。P的取值范围从0（贪</li>
</ul>
<p>婪解码）到1（LLM词汇表中的所有词元）。</p>
<p>在top‑K和top‑P之间选择的最佳方法是同时尝试这两种方法（或两者结合），看看哪种方</p>
<p>法能产生你想要的结果。</p>
<p>综合应用</p>
<p>在 top‑K、top‑P、温度和生成的令牌数量之间进行选择，取决于具体的应用和期望的结果，</p>
<p>这些设置彼此都会相互影响。了解所选模型如何结合不同的采样设置也非常重要。</p>
<p>如果温度、top‑K 和 top‑P 都可用（如在 Vertex Studio 中），满足 top‑K 和 top‑P 条件的令</p>
<p>牌将成为下一个预测令牌的候选，然后应用温度从通过 top‑K 和 top‑P 条件的令牌中进行采样。</p>
<p>如果只有 top‑K 或 top‑P 可用，行为相同，但只使用其中一个 top‑K 或 top‑P 设置。</p>
<p>如果温度不可用，则从满足 top‑K 和/或 top‑P 条件的令牌中随机选择，以生成单个下一个预测</p>
<p>令牌。</p>
<p>在某个采样配置值的极端设置下，该采样设置要么抵消其他配置设置，要么变得无关紧要。</p>
<p>年2月 11</p>
<ul>
<li>如果将温度设置为0，top‑K和top‑P将变得无关紧要——最可能的标记将成为下一个预</li>
</ul>
<p>测的标记。如果将温度设置得非常高（高于1——通常达到10以上），温度将变得无关紧要，</p>
<p>任何通过top‑K和/或top‑P标准的标记将被随机采样以选择下一个预测的标记。</p>
<ul>
<li>如果将top‑K设置为1，温度和top‑P将变得无关紧要。只有一个标记通过top‑K标准，该</li>
</ul>
<p>标记即为下一个预测的标记。如果将top‑K设置得非常高，比如达到大型语言模型词汇表的大</p>
<p>小，任何具有非零概率成为下一个标记的标记都将满足top‑K标准，没有被筛选掉。</p>
<ul>
<li>如果将top‑P设置为0（或非常小的值），大多数大型语言模型采样实现将只考虑最可能的</li>
</ul>
<p>标记以满足top‑P标准，使温度和top‑K变得无关紧要。如果将top‑P设置为1，任何具有非零</p>
<p>概率成为下一个标记的标记都将满足top‑P标准，没有被筛选掉。</p>
<p>作为一个通用的起点，温度设为0.2，top‑P设为0.95，top‑K设为30，可以获得相对连贯且具</p>
<p>有一定创造性的结果，但不会过于夸张。如果你想要特别有创造性的结果，可以尝试将温度设为</p>
<p>，top‑P设为0.99，top‑K设为40。如果你想要较少创造性的结果，可以尝试将温度设为</p>
<p>，top‑P设为0.9，top‑K设为20。最后，如果你的任务总是有唯一正确答案（例如，解答数</p>
<p>学问题），请将温度设为0。</p>
<p>注意：自由度越高（温度、top‑K、top‑P和输出标记越高），大型语言模型生成的文本可</p>
<p>能越不相关。</p>
<p>警告：你是否见过回答以大量填充词结尾的情况？这也被称为“重复循环错误”，这是大型</p>
<p>语言模型中的一个常见问题，模型会陷入循环，反复生成相同的（填充）词、短语或句子结构，</p>
<p>通常由于温度和top‑k设置不当而加剧。</p>
<p>年2月 12</p>`,
        'en': `<p>deterministic: the highest probability token is always selected (though note that if two tokens</p>
...
<p>The Gemini temperature control can be understood in a similar way to the softmax function Gemini</p>
<p>used in machine learning. A low temperature setting mirrors a low softmax temperature (T),</p>
<p>emphasizing a single, preferred temperature with high certainty. A higher Gemini temperature softmax</p>
<p>setting is like a high softmax temperature, making a wider range of temperatures around</p>
<p>the selected setting more acceptable. This increased uncertainty accommodates scenarios</p>
<p>where a rigid, precise temperature may not be essential like for example when experimenting</p>
<p>with creative outputs.</p>
<p>Top-K and top-P Top‑K</p>
<p>Top-K and top-P (also known as nucleus sampling)4 are two sampling settings used in LLMs Top‑K</p>
...
<p>generated text.</p>
<ul>
<li>Top-K sampling selects the top K most likely tokens from the model’s predicted • Top‑K</li>
</ul>
<p>distribution. The higher top-K, the more creative and varied the model’s output; the</p>
<p>lower top-K, the more restive and factual the model’s output. A top-K of 1 is equivalent to</p>
<p>greedy decoding.</p>
<p>February 2025 10 2025</p>
<ul>
<li>Top-P sampling selects the top tokens whose cumulative probability does not exceed • Top‑P</li>
</ul>
...
<p>cancels out other configuration settings or becomes irrelevant.</p>
<p>February 2025 11 2025</p>
<ul>
<li>If you set temperature to 0, top-K and top-P become irrelevant–the most probable •</li>
</ul>
...
<p>predicted token.</p>
<ul>
<li>If you set top-K to 1, temperature and top-P become irrelevant. Only one token passes the •</li>
</ul>
...
<p>next token will meet the top-K criteria and none are selected out.</p>
<ul>
<li>If you set top-P to 0 (or a very small value), most LLM sampling implementations will then •</li>
</ul>
...
<p>creative results, try starting with a temperature of .9, top-P of .99, and top-K of 40. And if you 0.9</p>
<p>want less creative results, try starting with a temperature of .1, top-P of .9, and top-K of 20. 0.1</p>
...
<p>WARNING: Have you ever seen a response ending with a large amount of filler words? This</p>
<p>is also known as the &quot;repetition loop bug&quot;, which is a common issue in Large Language</p>
<p>Models where the model gets stuck in a cycle, repeatedly generating the same (filler) word,</p>
<p>phrase, or sentence structure, often exacerbated by inappropriate temperature and top-k/</p>
<p>February 2025 12 2025</p>`,
      }
    },
    {
      title: 'top-p settings. This can occur at both l',
      title_i18n: {
        'zh-CN': 'top-p settings. This can occur at both l',
        'en': 'top-p settings. This can occur at both l',
      },
      level: 1,
      content: `<p>Prompt Engineering Prompt Engineering</p>
...
,
      content_i18n: {
        'zh-CN': `<p>设置。这种情况可能出现在低温和高温设置下，但原因不同。在低温时，模型变得过于确</p>
<p>定性，严格遵循最高概率路径，如果该路径重复访问之前生成的文本，可能导致循环。相反，在</p>
<p>高温时，模型的输出变得过于随机，增加了随机选择的单词或短语偶然回到先前状态的概率，由</p>
<p>于可选项众多，导致循环。在这两种情况下，模型的采样过程都会“卡住”，产生单调且无用</p>
<p>的输出，直到输出窗口填满。解决这个问题通常需要仔细调整温度和 top‑k/top‑p 值，以找到确</p>
<p>定性和随机性之间的最佳平衡。</p>
<p>提示技巧</p>
<p>大型语言模型（LLM）经过调优以遵循指令，并在大量数据上训练，因此它们能够理解提示并</p>
<p>生成答案。但大型语言模型并不完美；提示文本越清晰，模型预测下一个可能文本的效果越好。</p>
<p>此外，利用大型语言模型的训练方式和工作原理的特定技巧，将帮助你从模型中获得相关的结果。</p>
<p>既然我们已经了解了什么是提示工程以及它的要求，让我们深入探讨一些最重要的提示技术示</p>
<p>例。</p>
<p>通用提示 / 零样本</p>
<p>零样本提示是最简单的提示类型。它只提供任务描述和一些文本供大型语言模型开始处理。</p>
<p>这个输入可以是任何内容：一个问题、故事的开头或指令。零样本的名称意味着“无示例”。</p>
<p>年2月 13</p>
<p>让我们使用 Vertex AI 中的 Vertex AI Studio（语言部分），它提供了一个测试提示的游乐</p>
<p>场。在表1中，您将看到一个用于分类电影评论的零样本提示示例。</p>
<p>下面使用的表格格式是记录提示的绝佳方式。您的提示很可能会经过多次迭代，才会最终进入</p>
<p>代码库，因此以有纪律、有结构的方式跟踪您的提示工程工作非常重要。本章后面“最佳实践”</p>
<p>部分（“记录各种提示尝试”）中，将详细介绍这种表格格式、跟踪提示工程工作的重要性</p>
<p>以及提示开发过程。</p>
<p>模型温度应设置为较低的数值，因为不需要创造力，我们使用 gemini‑pro 默认的 top‑K 和</p>
<p>值，这实际上禁用了这两个设置（见上文“LLM 输出配置”）。请注意生成的输出。</p>
<p>单词 disturbing 和 masterpiece 应该会使预测稍微复杂一些，因为这两个词都出现在同一句</p>
<p>话中。</p>
<p>年2月 14</p>
<p>电影分类</p>
...
<p>输出 积极</p>
<p>表1. 零样本提示的示例</p>
<p>当零样本方法不起作用时，你可以在提示中提供演示或示例，这就形成了“一次示例”和</p>
<p>少量示例”提示。通用提示 / 零样本</p>
<p>一次示例与少量示例</p>
<p>在为AI模型创建提示时，提供示例是有帮助的。这些示例可以帮助模型理解你的需求。当你想</p>
<p>引导模型生成特定的输出结构或模式时，示例尤其有用。</p>
<p>一次示例提示提供一个单一的示例，因此得名一次示例。其理念是模型有一个示例可以模仿，</p>
<p>从而更好地完成任务。</p>
<p>少量示例提示向模型提供多个示例。这种方法向模型展示了它需要遵循的模式。理念与一次示</p>
<p>例类似，但多个期望模式的示例增加了模型遵循该模式的可能性。</p>
<p>年2月 15</p>`,
        'en': `<p>top-p settings. This can occur at both low and high temperature settings, though for different top‑p</p>
<p>reasons. At low temperatures, the model becomes overly deterministic, sticking rigidly to the</p>
<p>highest probability path, which can lead to a loop if that path revisits previously generated</p>
<p>text. Conversely, at high temperatures, the model&#x27;s output becomes excessively random,</p>
<p>increasing the probability that a randomly chosen word or phrase will, by chance, lead back</p>
<p>to a prior state, creating a loop due to the vast number of available options. In both cases,</p>
<p>the model&#x27;s sampling process gets &quot;stuck,&quot; resulting in monotonous and unhelpful output</p>
<p>until the output window is filled. Solving this often requires careful tinkering with temperature</p>
<p>and top-k/top-p values to find the optimal balance between determinism and randomness.</p>
...
<p>techniques that take advantage of how LLMs are trained and how LLMs work will help you get</p>
<p>the relevant results from LLMs</p>
<p>Now that we understand what prompt engineering is and what it takes, let’s dive into some</p>
...
<p>use the gemini-pro default top-K and top-P values, which effectively disable both settings top‑P</p>
...
<p>words are used in the same sentence.</p>
<p>February 2025 14 2025</p>
<p>Name 1_1_movie_classification Name 1_1_</p>
<p>Goal Classify movie reviews as positive, neutral or negative. Goal</p>
<p>Model gemini-pro</p>
<p>Temperature 0.1 Token Limit 5</p>
<p>Prompt Classify movie reviews as POSITIVE, NEUTRAL or NEGATIVE.</p>
<p>Review: &quot;Her&quot; is a disturbing study revealing the direction Review: &quot;Her&quot; is a disturbing study revealing the direction</p>
<p>Sentiment: Sentiment:</p>
<p>Output POSITIVE</p>
...
<p>A few-shot prompt 7 provides multiple examples to the model. This approach shows the</p>
<p>model a pattern that it needs to follow. The idea is similar to one-shot, but multiple examples</p>
<p>of the desired pattern increases the chance the model follows the pattern.</p>
<p>February 2025 15 2025</p>`,
      }
    }
  ]
};
